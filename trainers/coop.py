...
class PromptLearner(nn.Module):
    """CoOp prompt learner main class"""
...
class CoOp(TrainerX):
    ...
    def build_model(self):
        ...
        if dcount > 1:
            print(
                f"Multiple GPUs detected (n_gpus={dcount}), use all of them!")
    ...
    def load_model(self, directory, epoch=None):
        if not directory:
            print("load_model() skipped as no pretrained model is given")
            return
        ...
        for name in names:
            ...
            print(f"Loading weights to {name}")
            print(f'from "{model_path}" (epoch = {epoch})')
